...
JavaScript에서 **`this`**는 매우 중요한 개념 중 하나이다. **`this`**는 현재 실행 중인 함수나 메서드가 어떤 **`객체`**에 바인딩되어 있는지를 가리키는 특별한 **`키워드`**이다.

1. **`call` 메서드:`call`** 메서드는 함수를 호출할 때 **`this`** 값을 설정합니다. 함수를 호출하는 동안 함수 내에서 **`this`**가 특정 객체를 가리키도록 지정할 수 있습니다. **`call`** 메서드는 첫 번째 매개변수로 설정하려는 **`this`** 값(객체)을 받고, 이어서 함수에 전달할 매개변수를 순서대로 받는다.
...
2. **`apply` 메서드:`apply`** 메서드는 **`call`**과 비슷하지만 매개변수를 배열로 받는다.함수에 전달할 매개변수가 배열에 담겨 있을 때 유용하다.
...
3. **`bind` 메서드:`bind`** 메서드는 함수의 **`this`** 값을 영구적으로 설정한다.**`bind`**를 사용하여 새로운 함수를 생성하며, 이 새로운 함수의 **`this`** 값은 원본 함수의 **`this`** 값을 변경하지 않습니다. 대신 새로운 함수가 호출될 때 항상 설정한 **`this`** 값을 가진다.
...
이러한 메서드들을 사용하여 함수의 **`this`** 값을 조작할 수 있으므로, 함수가 다양한 객체와 함께 동작하도록 할 수 있다.

[단방향 바인딩과 양방향 바인딩](https://velog.io/@kimju0913/단방향-바인딩과-양방향-바인딩)

양방향 바인딩과 단방향 바인딩은 데이터와 사용자 인터페이스 간의 관계를 설명하는 두 가지 중요한 개념이다. 이 둘은 데이터의 변경이 사용자 인터페이스에 어떻게 반영되는지에 대한 접근 방식을 나타낸다.

1. **단방향 바인딩 (One-Way Binding):**
    - 단방향 바인딩은 데이터의 흐름이 한 방향으로만 이루어지는 바인딩입니다. 주로 데이터 모델에서 사용자 인터페이스로의 데이터 흐름 또는 데이터 소스에서 뷰로의 데이터 흐름을 나타냅니다.
    - 데이터 변경이 데이터 소스에서 시작하여 사용자 인터페이스로 전달되고, 사용자 인터페이스에서의 변경은 데이터 소스에 영향을 주지 않습니다.
    - 주로 정적인 웹 페이지 렌더링에 사용되며, React와 같은 라이브러리에서 많이 볼 수 있습니다.
    
    예를 들어, React 컴포넌트에서는 데이터가 상태(state)로 정의되고, 이 상태가 컴포넌트의 렌더링에 사용됩니다. 사용자가 뷰를 조작해도 데이터 소스에 변경을 알리지 않고 컴포넌트가 다시 렌더링됩니다.
    
2. **양방향 바인딩 (Two-Way Binding):**
    - 양방향 바인딩은 데이터와 사용자 인터페이스 간의 양방향 흐름을 허용하는 바인딩입니다. 즉, 데이터의 변경이 사용자 인터페이스에 반영되고, 사용자 인터페이스에서의 변경이 데이터에도 반영됩니다.
    - 이러한 바인딩은 주로 동적인 데이터 입력이나 폼 처리와 같은 상호 작용이 필요한 사용자 인터페이스에서 사용됩니다.
    - Angular와 Vue.js와 같은 프레임워크에서 양방향 바인딩이 지원됩니다.
    
    예를 들어, Angular에서는 ngModel 디렉티브를 사용하여 입력 요소와 데이터 모델을 양방향으로 바인딩할 수 있습니다. 입력 요소에서 값을 변경하면 데이터 모델도 자동으로 업데이트되며, 데이터 모델을 변경하면 입력 요소에도 반영됩니다.
    

양방향 바인딩은 사용자 경험을 향상시키지만 복잡성을 증가시킬 수 있으므로 상황에 맞게 선택해야 합니다. 단방향 바인딩은 데이터 흐름이 명확하고 예측 가능하며, 양방향 바인딩은 데이터와 뷰 간의 실시간 동기화를 제공합니다.
...
![스크린샷 2023-09-06 오전 12 01 28](https://github.com/in-ch/medipot_front/assets/49556566/efb7205a-f08f-4692-a80a-589bfedf4aa7)
...
![스크린샷 2023-09-05 오후 11 53 52](https://github.com/in-ch/medipot_front/assets/49556566/b3936eb2-5b84-45c7-87f0-faf11d3c3863)
